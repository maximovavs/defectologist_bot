...
SOFT_SKIP_REASONS = {
    "bad_candidate_url", "skip_non_html_asset", "dup_url_same_run", "dup_url_db", "no_evidence_short",
    "dup_evidence_same_run", "dup_evidence_hash_db", "dup_semantic_source", "dup_body_same_run",
    "dup_body_hash_db", "dup_semantic_post", "rubric_topic_mismatch_source", "rubric_topic_mismatch_post",
    "tip_of_day_post_too_generic", "unknown_source_id", "llm_invalid_output", "no_candidates", "max_skips_per_rubric",
}
HARD_SKIP_REASONS = {
    "source_fetch_failed", "evidence_fetch_failed", "llm_timeout", "llm_failed", "visual_build_failed",
    "telegram_send_failed", "max_run_seconds",
}

AGE_NORMS_BAD_MARKERS = ["дисграф", "дислекс", "заикан", "алал", "афаз", "дизартр", "ринолал", "коррекц", "нарушени", "дефект", "диагноз", "патолог"]
AGE_NORMS_GOOD_MARKERS = ["возраст", "норма", "ориентир", "milestone", "development", "communication", "речевое развитие", "что умеет", "что обычно"]
TIP_OF_DAY_BAD_MARKERS = ["совет логопеда дня", "сегодня работаем над", "сегодня поговорим", "развитие речи", "общее недоразвитие речи", "онр", "дизартр", "алали", "дисграф", "дислекс", "диагноз", "коррекц"]
TIP_OF_DAY_GOOD_MARKERS = ["👶 возраст:", "🧩 что попробовать сегодня", "👄 пример", "💡 что это дает"]
...
def _resolve_publication_db_path() -> Path:
    return STATE_DIR / ("publication_history_test.sqlite3" if _resolve_state_scope() == "test" else "publication_history.sqlite3")
...
def _build_posted_zero_alert_plain(now: datetime, day: str, week_key: str, audience: str, provider: str,
                                   soft_skip_reasons: Dict[str, int], hard_skip_reasons: Dict[str, int],
                                   samples: List[str], state_scope: str, db_name: str,
                                   attempted_rubrics: List[str]) -> str:
...
def _strip_html_tags_for_telegram(text: str) -> str:
    ...
    s = re.sub(r'<a\s+href="([^"]+)">(.+?)</a>', lambda m: f"{_html.unescape(m.group(2))} ({_html.unescape(m.group(1))})", s, flags=re.IGNORECASE | re.DOTALL)
...
def _log_telegram_html_fallback(context: str, html_text: str, exc: Exception) -> None:
    print(f"[WARN] telegram_html_fallback context={context} err={exc} html_preview={_safe_html_preview(html_text)}", flush=True)


def _line_matches_structural(st: str) -> bool:
    return any((AGE_LINE_RE.match(st), AUDIENCE_LINE_RE.match(st), SOURCE_LINE_RE.match(st), BENEFIT_LINE_RE.match(st), MYTH_LINE_RE.match(st), QUESTION_LINE_RE.match(st), ORIENTIRS_LINE_RE.match(st), GAME_HEADING_RE.match(st), TRY_TODAY_HEADING_RE.match(st), BILINGUAL_HEADING_RE.match(st), HOME_HEADING_RE.match(st), EXAMPLE_HEADING_RE.match(st), BENEFIT_HEADING_RE.match(st)))
...
def _remove_footer_lines(lines: List[str]) -> List[str]:
    cleaned = [line for line in lines if not SOURCE_LINE_RE.match(line.strip()) and not line.strip().startswith("🔗 ")]
...
def finalize_plain_post_for_publication(plain_text: str, day_key: str, source_domain: str, source_url: str, max_chars: int) -> str:
...
def _extract_cover_title_from_plain_post(plain_text: str, fallback: str, rubric_title: str = "") -> str:
    ...
    for line in lines:
        ...
        if not st or st.startswith("#") or SOURCE_LINE_RE.match(st) or st.startswith("🔗 ") or AGE_LINE_RE.match(st) or AUDIENCE_LINE_RE.match(st) or _is_structural_heading(st):
            continue
...
def _is_age_norms_content_fit(text: str) -> bool:
    blob = (text or "").lower().replace("ё", "е")
    return not _contains_any_marker(blob, AGE_NORMS_BAD_MARKERS) and _contains_any_marker(blob, AGE_NORMS_GOOD_MARKERS)
...
def _is_tip_of_day_content_fit(text: str) -> bool:
    ...
    if "совет логопеда дня" in title:
        return False
    blob = (text or "").lower().replace("ё", "е")
    return (not _contains_any_marker(blob, TIP_OF_DAY_BAD_MARKERS)) and _contains_any_marker(blob, TIP_OF_DAY_GOOD_MARKERS)
...
def fetch_rss(url: str) -> List[Dict[str, str]]:
    ...
    return [{"title": norm_space(getattr(e, "title", "")), "link": getattr(e, "link", ""), "summary": norm_space(re.sub("<.*?>", "", getattr(e, "summary", "")))} for e in d.entries[:50]]
...
def _collect_links(base_url: str, soup: BeautifulSoup, selector: str, href_re: Optional[str] = None) -> List[Dict[str, str]]:
    ...
    seen: set[str] = set(); uniq: List[Dict[str, str]] = []
    for it in out:
        if it["link"] in seen:
            continue
        seen.add(it["link"])
...
def parse_logorina_news(url: str, html_text: str) -> List[Dict[str, str]]:
    ...
    return [it for it in items if not re.search(r"/news/\d{4}-\d{2}/?$", it.get("link", ""))][:80]
...
SITE_PARSERS = {"logorina_news": parse_logorina_news, "logomag_lib": parse_logomag_lib, "logoportal_articles": parse_logoportal_articles, "logopedy_articles": parse_logopedy_articles, "logopediya_publ": parse_logopediya_publ}


def fetch_html_site(url: str, parser_name: str) -> List[Dict[str, str]]:
    r = requests.get(url, headers=HEADERS, timeout=30, verify=_verify_for_url(url)); r.raise_for_status()
...
def fetch_source(src: Source) -> List[Dict[str, str]]:
    if src.type == "rss": return fetch_rss(src.url or "")
    if src.type == "static": return fetch_static(src.urls or [])
    if src.type == "html_site": return fetch_html_site(src.url or "", src.parser or "")
    raise ValueError(f"Unsupported source type: {src.type}")


def get_canonical(url: str) -> str:
    try:
        r = requests.get(url, headers=HEADERS, timeout=25, verify=_verify_for_url(url)); r.raise_for_status()
...
def extract_evidence_text(url: str, max_chars: int = 3600) -> str:
    r = requests.get(url, headers=HEADERS, timeout=35, verify=_verify_for_url(url)); r.raise_for_status()
    ...
    chunks: List[str] = []
    h1 = soup.find("h1")
    if h1: chunks.append(norm_space(h1.get_text(" ", strip=True)))
    for el in root.select("h2, h3, p, li"):
        txt = norm_space(el.get_text(" ", strip=True))
        if len(txt) < 20: continue
        low = txt.lower()
        if any(bad in low for bad in ["cookie", "privacy", "политик", "подпис", "реклама", "скачать", "регистрация"]): continue
        chunks.append(txt)
        if sum(len(x) for x in chunks) > max_chars * 1.35: break
    ...
    out = "\n".join(uniq).strip()
    if len(out) > max_chars: out = out[:max_chars].rsplit("\n", 1)[0].strip()
    return out
...
def render_plain_to_telegram_html(plain_text: str) -> str:
    ...
    def _link_anchor(url: str, prefix: str = "🔗 ") -> str:
        label = "Читать оригинальный материал"; href = _html.escape(url, quote=True)
        return f'{prefix}<a href="{href}">{_escape(label)}</a>'
    out: List[str] = []
    for idx, raw in enumerate(lines):
        s = raw.rstrip("\n"); st = s.strip()
        if idx == 0 and st:
            out.append(f"<b>{_escape(st)}</b>"); continue
        ...
        if st.startswith("🔗 "):
            ...
            continue
        if st.startswith("ℹ️ "):
            out.append(f"<i>{_escape(st)}</i>"); continue
...
def tg_request(method: str, data: Dict[str, Any], files: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not r.ok:
        ...
        if not description: description = r.text or ""
...
def send_message(chat_id: str, html_text: str) -> None:
    ...
    base_data: Dict[str, Any] = {"chat_id": chat_id, "text": html_text, "disable_web_page_preview": "true"}
    if TELEGRAM_PARSE_MODE:
        try:
            data = dict(base_data); data["parse_mode"] = TELEGRAM_PARSE_MODE; tg_request("sendMessage", data=data); return
        except Exception as e:
            if not _is_probably_parse_mode_error(e): raise
            _log_telegram_html_fallback("send_message", html_text, e)
    fallback_text = _strip_html_tags_for_telegram(html_text)
    tg_request("sendMessage", data={"chat_id": chat_id, "text": fallback_text, "disable_web_page_preview": "true"})


def send_plain_message(chat_id: str, text: str) -> None:
    if not chat_id: raise RuntimeError("chat_id is missing")
    tg_request("sendMessage", data={"chat_id": chat_id, "text": text, "disable_web_page_preview": "true"})
...
def send_post_with_visual(chat_id: str, photo_buffer: BytesIO, plain_post: str, html_full_post: str) -> None:
    ...
    if plain_bytes <= TG_CAPTION_MAX_BYTES:
        try:
            ...
            if TELEGRAM_PARSE_MODE: data["parse_mode"] = TELEGRAM_PARSE_MODE
            tg_request("sendPhoto", data=data, files={"photo": file_tuple}); return
        except Exception as e:
            if _is_probably_parse_mode_error(e): _log_telegram_html_fallback("send_photo_caption", html_full_post, e)
            else: print(f"[WARN] send_photo_with_caption_failed err={e}", flush=True)
    tg_request("sendPhoto", data={"chat_id": chat_id, "caption": ""}, files={"photo": file_tuple})
    send_message(chat_id, html_full_post)


def send_semantic_alert(chat_id: str, candidate_url: str, matched_url: str, score: float, audience: str, rubric_id: str, match_field: str) -> None:
    plain_text = ("⚠️ Semantic dedup alert\n" f"Материал отклонён: cosine similarity ≥ {SEMANTIC_THRESHOLD:.2f}\n" f"AUDIENCE={audience} | RUBRIC={rubric_id} | FIELD={match_field}\n\n" f"Новый кандидат: {candidate_url}\n" f"Похож на: {matched_url}\n" f"Cosine: {score:.3f}")
...
async def amain() -> None:
    ...
    if RESET_TEST_DB and state_scope == "test":
        try:
            if db_path.exists(): db_path.unlink(); print(f"[RESET_TEST_DB] removed {db_path}", flush=True)
            else: print(f"[RESET_TEST_DB] file not found: {db_path}", flush=True)
        except Exception as e:
            print(f"[RESET_TEST_DB][WARN] failed to remove {db_path}: {e}", flush=True)
    print(f"[START] Publisher started at {now.isoformat()} target_channel={TARGET_CHANNEL} state_scope={state_scope} db={db_path.name} rubric_id={selected_rubric_id or '(auto)'} reset_test_db={RESET_TEST_DB}", flush=True)
    ...
    aud_list = ["parents", "pros"] if AUDIENCE == "both" else ([AUDIENCE] if AUDIENCE in ("parents", "pros") else ["parents"])
    posted = 0
    ...
    def note(reason: str, url: str) -> str:
        ...
        (hard_skip_reasons if kind == "hard" else soft_skip_reasons)[reason] = (hard_skip_reasons if kind == "hard" else soft_skip_reasons).get(reason, 0) + 1
        if len(samples) < 10: samples.append(f"[{kind}] {reason}: {url}")
        return kind
    ...
    for aud in aud_list:
        ...
        for rubric in rubrics:
            ...
            if rf == "quality_dashboard": continue
            ...
            if selected_rubric_id:
                if rubric_id.lower() != selected_rubric_id: continue
            else:
                if not is_due(rubric, now): continue
            if rubric_id not in attempted_rubrics: attempted_rubrics.append(rubric_id)
            rubric_skips = 0
            selected_sources = list(rubric.get("sources", []) or [])
            if INCLUDE_SOURCES:
                include_set = set(INCLUDE_SOURCES); selected_sources = [sid for sid in selected_sources if sid in include_set]
            if EXCLUDE_SOURCES:
                exclude_set = set(EXCLUDE_SOURCES); selected_sources = [sid for sid in selected_sources if sid not in exclude_set]
            print(f"[RUBRIC_SOURCES] rubric={rubric_id} selected_sources={selected_sources}", flush=True)
            all_items: List[Dict[str, str]] = []
            for sid in selected_sources:
                src = sources.get(sid)
                if not src:
                    kind = note("unknown_source_id", sid); print(f"[SKIP][{kind}] unknown_source_id id={sid}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    continue
                try: all_items.extend(fetch_source(src))
                except Exception as e:
                    kind = note("source_fetch_failed", f"{sid}: {e}"); print(f"[SKIP][{kind}] source_fetch_failed source={sid} err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
            if not all_items:
                note("no_candidates", rubric_id); continue
            seed = int(hashlib.sha1(f"{now.date()}|{rubric_id}|{aud}".encode("utf-8")).hexdigest()[:8], 16)
            rng = random.Random(seed); rng.shuffle(all_items)
            print(f"[RUBRIC] rubric={rubric_id} audience={aud} candidates_total={len(all_items)} max_scan={MAX_CANDIDATES_PER_RUBRIC}", flush=True)
            for cand in all_items[:MAX_CANDIDATES_PER_RUBRIC]:
                ...
                if elapsed > MAX_RUN_SECONDS:
                    kind = note("max_run_seconds", rubric_id); print(f"[STOP][{kind}] max_run_seconds reached: {elapsed:.1f}s", flush=True); break
                print(f"[CANDIDATE] rubric={rubric_id} audience={aud} url={url}", flush=True)
                if not url.startswith(("http://", "https://")):
                    kind = note("bad_candidate_url", url or "(empty)"); print(f"[SKIP][{kind}] bad_candidate_url url={url}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if _SKIP_EXT_RE.search(url):
                    kind = note("skip_non_html_asset", url); print(f"[SKIP][{kind}] skip_non_html_asset url={url}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                canon = get_canonical(url)
                if _SKIP_EXT_RE.search(canon):
                    kind = note("skip_non_html_asset", canon); print(f"[SKIP][{kind}] skip_non_html_asset canon={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if canon in seen_urls_this_run:
                    kind = note("dup_url_same_run", canon); print(f"[SKIP][{kind}] dup_url_same_run url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if store.has_url(canon):
                    kind = note("dup_url_db", canon); print(f"[SKIP][{kind}] dup_url_db url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                try: evidence = extract_evidence_text(canon, max_chars=3600)
                except Exception as e:
                    kind = note("evidence_fetch_failed", f"{canon} ({e})"); print(f"[SKIP][{kind}] evidence_fetch_failed url={canon} err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if len((evidence or "").strip()) < 260:
                    kind = note("no_evidence_short", canon); print(f"[SKIP][{kind}] no_evidence_short url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if rubric_id == "age_norms" and not _is_age_norms_content_fit(evidence):
                    kind = note("rubric_topic_mismatch_source", canon); print(f"[SKIP][{kind}] rubric_topic_mismatch_source url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                evidence_hash = sha1(norm_space(evidence))
                if evidence_hash in seen_evidence_hashes_this_run:
                    kind = note("dup_evidence_same_run", canon); print(f"[SKIP][{kind}] dup_evidence_same_run url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if store.has_evidence_hash(evidence_hash):
                    kind = note("dup_evidence_hash_db", canon); print(f"[SKIP][{kind}] dup_evidence_hash_db url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                sem_source_hit = store.find_semantic_duplicate(evidence, threshold=SEMANTIC_THRESHOLD, since_iso=None, limit=500, compare="evidence")
                if sem_source_hit:
                    kind = note("dup_semantic_source", canon); print(f"[SKIP][{kind}] dup_semantic_source url={canon} matched={sem_source_hit.canonical_url} score={sem_source_hit.similarity:.3f}", flush=True)
                    if not DRY_RUN and TELEGRAM_DRAFTS_CHAT_ID:
                        recent_hit = store.find_semantic_duplicate(evidence, threshold=SEMANTIC_THRESHOLD, since_iso=recent_since_iso, limit=120, compare="evidence")
                        if recent_hit:
                            try: send_semantic_alert(TELEGRAM_DRAFTS_CHAT_ID, canon, recent_hit.canonical_url, recent_hit.similarity, aud, rubric_id, recent_hit.match_field)
                            except Exception as e: print(f"[WARN] failed_to_send_semantic_alert err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                sd = safe_domain(canon) or safe_domain(url) or "источник"
                try:
                    plain_raw, ok, llm_note = await asyncio.wait_for(generate_post_plain_from_evidence_async(rubric_title=rubric_title, rubric_format=rf, audience=aud, title_suffix=title_suffix, source_domain=sd, source_url=canon, evidence_text=evidence, disclaimer=disclaimer, hashtags=hashtags if aud != "pros" else [], provider=PROVIDER, groq_key=GROQ_API_KEY, gemini_key=GEMINI_API_KEY, max_chars=POST_MAX_CHARS, day_key=effective_day), timeout=MAX_LLM_SECONDS_PER_CANDIDATE)
                except asyncio.TimeoutError:
                    kind = note("llm_timeout", canon); print(f"[SKIP][{kind}] llm_timeout url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                except Exception as e:
                    kind = note("llm_failed", f"{canon} ({e})"); print(f"[SKIP][{kind}] llm_failed url={canon} err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if not ok or not plain_raw:
                    kind = note("llm_invalid_output", canon); print(f"[SKIP][{kind}] {llm_note} url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                plain = finalize_plain_post_for_publication(plain_text=plain_raw, day_key=effective_day, source_domain=sd, source_url=canon, max_chars=POST_MAX_CHARS)
                if rubric_id == "age_norms" and not _is_age_norms_content_fit(plain):
                    kind = note("rubric_topic_mismatch_post", canon); print(f"[SKIP][{kind}] rubric_topic_mismatch_post url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if rubric_id == "tip_of_day" and not _is_tip_of_day_content_fit(plain):
                    kind = note("tip_of_day_post_too_generic", canon); print(f"[SKIP][{kind}] tip_of_day_post_too_generic url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                body_hash = sha1(norm_space(plain))
                if body_hash in seen_body_hashes_this_run:
                    kind = note("dup_body_same_run", canon); print(f"[SKIP][{kind}] dup_body_same_run url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                if store.has_body_hash(body_hash):
                    kind = note("dup_body_hash_db", canon); print(f"[SKIP][{kind}] dup_body_hash_db url={canon}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                sem_body_hit = store.find_semantic_duplicate(plain, threshold=SEMANTIC_THRESHOLD, since_iso=None, limit=500, compare="body")
                if sem_body_hit:
                    kind = note("dup_semantic_post", canon); print(f"[SKIP][{kind}] dup_semantic_post url={canon} matched={sem_body_hit.canonical_url} score={sem_body_hit.similarity:.3f}", flush=True)
                    if not DRY_RUN and TELEGRAM_DRAFTS_CHAT_ID:
                        recent_post_hit = store.find_semantic_duplicate(plain, threshold=SEMANTIC_THRESHOLD, since_iso=recent_since_iso, limit=120, compare="body")
                        if recent_post_hit:
                            try: send_semantic_alert(TELEGRAM_DRAFTS_CHAT_ID, canon, recent_post_hit.canonical_url, recent_post_hit.similarity, aud, rubric_id, recent_post_hit.match_field)
                            except Exception as e: print(f"[WARN] failed_to_send_semantic_alert err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                h1_title = _extract_cover_title_from_plain_post(plain, fallback=rubric_title, rubric_title=rubric_title)
                image_prompt = ""; image_prompt_note = "skipped"
                try:
                    image_prompt, image_prompt_ok, image_prompt_note = await asyncio.wait_for(generate_image_prompt_async(title=h1_title, body_text=plain, audience=aud, provider=PROVIDER, groq_key=GROQ_API_KEY, gemini_key=GEMINI_API_KEY), timeout=IMAGE_PROMPT_TIMEOUT_SECONDS)
                    if not image_prompt_ok: image_prompt = ""
                except asyncio.TimeoutError:
                    image_prompt = ""; image_prompt_note = "image_prompt_timeout"
                except Exception as e:
                    image_prompt = ""; image_prompt_note = f"image_prompt_failed:{e}"
                try:
                    visual_buffer, visual_meta = build_post_visual(title=h1_title, day_key=effective_day, image_prompt=image_prompt, pollinations_token=POLLINATIONS_TOKEN)
                except Exception as e:
                    kind = note("visual_build_failed", f"{canon} ({e})"); print(f"[SKIP][{kind}] visual_build_failed url={canon} err={e}", flush=True)
                    if kind == "hard": rubric_skips += 1
                    if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                    continue
                print(f"[VISUAL] rubric={rubric_id} mode={visual_meta.get('mode')} reason={visual_meta.get('reason')} image_prompt_note={image_prompt_note}", flush=True)
                html_full = render_plain_to_telegram_html(plain)
                if DRY_RUN:
                    ts = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
                    out = STATE_DIR / "dry_run" / ts; out.mkdir(parents=True, exist_ok=True)
                    ...
                    if not target_chat_id: raise RuntimeError("Resolved target chat id is empty")
                    try: send_post_with_visual(target_chat_id, visual_buffer, plain, html_full)
                    except Exception as e:
                        kind = note("telegram_send_failed", f"{canon} ({e})"); print(f"[SKIP][{kind}] telegram_send_failed url={canon} err={e}", flush=True)
                        if kind == "hard": rubric_skips += 1
                        if rubric_skips >= MAX_SKIPS_PER_RUBRIC: note("max_skips_per_rubric", rubric_id); print(f"[STOP] max_skips_per_rubric reached for {rubric_id}", flush=True); break
                        continue
                    store.record_publication(canonical_url=canon, body_hash=body_hash, body_text=plain, evidence_hash=evidence_hash, evidence_text=evidence, posted_at=now.isoformat(), audience=aud, rubric_id=rubric_id, rubric_title=rubric_title, source_domain=sd)
                seen_urls_this_run.add(canon); seen_body_hashes_this_run.add(body_hash); seen_evidence_hashes_this_run.add(evidence_hash)
                posted += 1; print(f"[POSTED] rubric={rubric_id} audience={aud} url={canon}", flush=True); await asyncio.sleep(1.0); break
            if (time.monotonic() - run_started_monotonic) > MAX_RUN_SECONDS or posted >= max_posts: break
        if (time.monotonic() - run_started_monotonic) > MAX_RUN_SECONDS or posted >= max_posts: break
    if posted == 0 and not DRY_RUN:
        if TELEGRAM_DRAFTS_CHAT_ID:
            try:
                send_plain_message(TELEGRAM_DRAFTS_CHAT_ID, _build_posted_zero_alert_plain(now=now, day=day, week_key=week_key, audience=AUDIENCE, provider=PROVIDER, soft_skip_reasons=soft_skip_reasons, hard_skip_reasons=hard_skip_reasons, samples=samples, state_scope=state_scope, db_name=db_path.name, attempted_rubrics=attempted_rubrics))
            except Exception as e:
    ...
    print(f"Publisher done. Posted: {posted}. Week: {week_key}.{' [DRY_RUN]' if DRY_RUN else ''}", flush=True)
