...
def _parse_env_csv_list(raw: str) -> List[str]:
...
INCLUDE_SOURCES = _parse_env_csv_list(os.getenv("INCLUDE_SOURCES", ""))
EXCLUDE_SOURCES = _parse_env_csv_list(os.getenv("EXCLUDE_SOURCES", ""))
...
AGE_NORMS_BAD_MARKERS = [
    "дисграф", "дислекс", "заикан", "алал", "афаз", "дизартр",
    "ринолал", "коррекц", "нарушени", "дефект", "диагноз", "патолог",
]
...
def _resolve_publication_db_path() -> Path:
    if _resolve_state_scope() == "test":
        return STATE_DIR / "publication_history_test.sqlite3"
    return STATE_DIR / "publication_history.sqlite3"


def _skip_kind(reason: str) -> str:
    if reason in HARD_SKIP_REASONS:
        return "hard"
    return "soft"
...
def _body_supports_tag(tag: str, body_text: str) -> bool:
    ...
    tag_tokens = [_normalize_tag_token(x) for x in tag_body.split() if _normalize_tag_token(x)]
...
async def amain() -> None:
    ...
    selected_include_sources = INCLUDE_SOURCES
    selected_exclude_sources = EXCLUDE_SOURCES
    ...
    def note(reason: str, url: str) -> str:
        ...
        if kind == "hard":
            hard_skip_reasons[reason] = hard_skip_reasons.get(reason, 0) + 1
        else:
            soft_skip_reasons[reason] = soft_skip_reasons.get(reason, 0) + 1
        if len(samples) < 10:
    ...
    for aud in aud_list:
        ...
        for rubric in rubrics:
            ...
            effective_day = rubric_days[0] if selected_rubric_id and rubric_days else day

            rubric_source_ids: List[str] = list(rubric.get("sources", []) or [])
            selected_source_ids: List[str] = []
            for sid in rubric_source_ids:
                sid_norm = sid.strip().lower()
                if selected_include_sources and sid_norm not in selected_include_sources:
                    continue
                if selected_exclude_sources and sid_norm in selected_exclude_sources:
                    continue
